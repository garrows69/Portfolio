/*************************************************************************
 *  BASED ON Princeton's StdAudio File
 *************************************************************************/
...
public class Beeper {
    ...
    private static byte[] readByte(String filename) {
        ...
        try {
            ...
            // try to read from URL
                ...
                URL url = Beeper.class.getResource(filename);
    ...
    ***********************************************************************/
    ...
    private static double[] note(double hz, double duration, double amplitude) {
        int N = (int) (Beeper.SAMPLE_RATE * duration);
        double[] a = new double[N+1];
        for (int i = 0; i <= N; i++)
            a[i] = amplitude * Math.sin(2 * Math.PI * i * hz / Beeper.SAMPLE_RATE);
        return a;
    }
    ...
    public static void main(String[] args) {
        ...
        // 440 Hz for 1 sec
        double freq = 440.0;
        for (int i = 0; i <= Beeper.SAMPLE_RATE; i++) {
            Beeper.play(0.5 * Math.sin(2*Math.PI * freq * i / Beeper.SAMPLE_RATE));
        }
...
***************************************/
	...
	public static void beep(int hz, int duration)
	{
		play(note(hz, duration/1000.0, 0.5));
		/*
		double[] a = new double[rate + 1];
		for (int i = 0; i <= rate; i++) {
			a[i] = Math.sin(2 * Math.PI * i * 440 / rate); 
		}
		play(a);
		*/ 
	}
