...
void LinkedList::printList(bool test)
{
	Node* currentNode = new Node;
	Node* previous = new Node;
	...
	if (head != NULL)
	{
		if (!test)
		{
			...
			{
				std::cout << "Id: " << currentNode->data.id << " Data: " << currentNode->data.data << std::endl;
				currentNode = currentNode->next;
			}
		}
		else
		{
			...
			for (int i = 0; i < count && currentNode != NULL; i++)
			{
				prev = currentNode;
				currentNode = currentNode->next;
			}
			...
			{
				std::cout << "Id: " << prev->data.id << " Data: " << prev->data.data << std::endl;
				prev = prev->prev;
			}
		}
	}
